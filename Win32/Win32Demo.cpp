...
//
// Msg Handler
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) //contents of msg variable basically
{
	int wmId, wmEvent;
	PAINTSTRUCT ps;
	HDC hdc;
	...
	case WM_COMMAND:
		...
		switch (wmId)
		{
		case IDM_ABOUT: //name of menu item
			DialogBox(hInst, MAKEINTRESOURCE(IDD_ABOUTBOX), hWnd, About);
			break;
		case IDM_EXIT:
			//TODO if (IDYES == ....)
			if (IDOK == MessageBox(0, L"Wanna Quit", L"Exit", MB_OK | MB_OKCANCEL))
				DestroyWindow(hWnd); //passed with wnd id: 1.clear look and feel 2:destroy
			break;
		default:
	...
	case WM_DESTROY:
		PostQuitMessage(0); // post the quit msg in AMQ (Indicates to the system that a thread has made a request to 
